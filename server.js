...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && pathname === "/analytics-event") {
    ...
    if (!["car_view", "garage_view", "call_click", "email_click", "directions_click"].includes(eventType)) {
      return sendJson(res, 400, {
        success: false,
    ...
    try {
      ...
      if (["car_view", "call_click", "email_click", "directions_click"].includes(eventType)) {
        if (!carId) {
          return sendJson(res, 400, {
            success: false,
